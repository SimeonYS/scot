...
class ScotPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('scot.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `scot`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM scot WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO scot (date, title, link, content)"
